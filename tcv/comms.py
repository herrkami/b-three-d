...
# This module is in large parts shamelessly copied from ocp_vscode/comms.py (see
# https://github.com/bernhard-42/vscode-ocp-cad-viewer). It only applies minor
# changes specific to VSCode to make it a little more generic:
# - .state now refers to b_three_d.state 
# - Default port number set to 3937
# - Slight adjustment of the info and error messages
...
# This is the patched version of .state
...
def _send(data, message_type, port=None, timeit=False):
    ...
    try:
        ...
        print(
            f"Cannot connect to viewer on port {port}. Is it running on the provided port?"
        )
        print(ex)
        return None
    ...
    def find_port():
        ...
        if port is None:
            ...
            if len(ports) == 1:
                ...
                raise RuntimeError(
                    f"\nMultiple ports found ({', '.join(ports)}) and the connection file is outside\n" 
                    "of any workspace folder of this Python instance."
                    "The specific viewer cannot be auto detected, please use set_port(port) in your\n"
                    "code."
                )
            else:
                raise RuntimeError(
...
def set_connection_file():
    """Set the connection file for Jupyter in b_three_d.state.CONFIG_FILE"""
