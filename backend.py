...
# This file is in large parts shamelessly copied from ocp_vscode/backend.py (see
...
# - .comms, .build123d are now imported from b_three_d
# - change some message strings
...
from .comms import MessageType, listener, send_response, set_port
...
class ViewerBackend:
    ...
    It's job is to send responses to the frontend application that goes through the three cad
    viewer view.
    The responses hold all the data needed to display the measurements.
    """
    ...
    def load_model(self, raw_model):
        ...
        self.model = {}
        trace = Trace("b-three-d-backend.log")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("b-three-d viewer backend")
    parser.add_argument(
